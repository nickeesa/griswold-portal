...
// value as an iframe src when it is exactly an https URL on either
// lookerstudio.google.com or datastudio.google.com (Google renamed Looker Studio
// back to Data Studio in April 2026, so newer report links use the datastudio
// domain while older ones still use lookerstudio). Any other origin or non-URL
// returns null and must NOT be embedded — this prevents an admin-entered field
// from framing an arbitrary origin even under the relaxed frame-src CSP.
  ...
  try {
    ...
    return u.protocol === 'https:' &&
      (u.hostname === 'lookerstudio.google.com' ||
        u.hostname === 'datastudio.google.com')
      ? u.toString()
      : null;
  } catch {
    return null;
